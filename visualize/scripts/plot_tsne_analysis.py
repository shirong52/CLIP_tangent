#!/usr/bin/env python3
"""t-SNE visualisation for triplet-based embedding analysis.
...
python plot_tsne_analysis.py --embeddings_path /root/CLIP_tangent/visualize/results/embeddings.npz
"""
...
def run_tsne(emb: np.ndarray, perplexity: int = 5, max_iter: int = 1000, random_state: int = 42):
...
def prepare_tsne_stack(arrays: dict, modality: str):
    """Stack anchor / floor_pert / obj_color_pert into one matrix for joint t-SNE."""
...
def plot(img2d, img_roles, img_gids,
    ...
    fig.suptitle(
        "Triplet Analysis (t-SNE): Floor-Color (large-area) vs Object-Color (small-area) Perturbations\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate t-SNE triplet plots and cosine-distance metrics."
    )
    parser.add_argument("--embeddings_path", default="results/embeddings.npz")
    parser.add_argument("--output_image",   default="results/tsne_analysis.png")
    parser.add_argument("--output_metrics", default="results/tsne_distance_metrics.json")
    ...
    # t-SNE
    print("\nPreparing t-SNE input ...")
    img_stack, img_roles, img_gids = prepare_tsne_stack(arrays, "image")
    txt_stack, txt_roles, txt_gids = prepare_tsne_stack(arrays, "text")

    print("Running t-SNE on image embeddings ...")
    img2d = run_tsne(img_stack, args.perplexity, args.max_iter)

    print("Running t-SNE on text embeddings ...")
    txt2d = run_tsne(txt_stack, args.perplexity, args.max_iter)
